...
const tip = (uploaded, total) => {
    ...
    return `Uploading to Tencent CloudBase: ${percentage == 0 ? '' : percentage + '% '}${uploaded}/${total} files uploaded`;
};
...
module.exports = class TcbPlugin {
    constructor(options) {
    ...
    apply(compiler) {
        compiler.plugin('after-emit', (compilation, callback) => {
            ...
            // Perform upload to tcb
            const performUpload = function(fileName) {
                let file = assets[fileName] || {};
                fileName = basePath + '/' + fileName.replace(/\\/g, '/');
                let cloudPath = path.posix.join(uploadPath, fileName);

                return new Promise((resolve, reject) => {
                    let begin = Date.now();
                    const res = app.uploadFile({
                        cloudPath: cloudPath,
                        fileContent: fs.createReadStream(file.existsAt)
                    });
